...
class User():
    ...
    def validate_reg(form_data):
        ...
        for field in form_data:
            ...
            if len(form_data['last_name']) < 3:
                ...
                is_valid = False
            if len(form_data['password']) < 8 and not password_regex.match(form_data['password']):
                flash("Password must be at least 8 characters and contain at least one uppercase letter, one lowercase letter, and one number!", "register")
                is_valid = False
    ...
    def validate_login(form_data):
        ...
        if not user:
            flash("Email has not be registered in database!", "login")
            return False
